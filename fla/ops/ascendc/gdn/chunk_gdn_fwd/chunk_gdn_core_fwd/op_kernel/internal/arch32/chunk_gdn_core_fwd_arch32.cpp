 ...
 */
#include "chunk_gdn_core_fwd_arch32_struct.h"
...
__aicore__ inline const __gm__ Arch32ChunkGdnCoreFwdTrailer *GetPhase6Trailer(GM_ADDR tiling)
{
    const uint64_t oTilingOffset = AlignPhase6(
        sizeof(ChunkGatedDeltaRuleFwdHTilingData), PHASE6_TILING_ALIGNMENT);
    const uint64_t phase5End = oTilingOffset + sizeof(ChunkFwdOTilingData) +
                               sizeof(ChunkRecomputeWUFwdHOTrailer);
    return reinterpret_cast<const __gm__ Arch32ChunkGdnCoreFwdTrailer *>(
        tiling + AlignPhase6(phase5End, PHASE6_TILING_ALIGNMENT));
}

__aicore__ inline void CopyAbcTiling(
    const __gm__ Arch32ChunkGdnCoreFwdAbcTiling *src, Arch32ChunkGdnCoreFwdAbcTiling &dst)
{
    dst.B = src->B;
    dst.Hk = src->Hk;
    dst.Hv = src->Hv;
...
__aicore__ inline void WritePublicCumsumRows(
    ...
    const Arch32ChunkGdnCoreFwdAbcTiling &tiling)
{
    if ASCEND_IS_AIC {
        return;
    }
    ...
    GM_ADDR finalState, GM_ADDR gCumsumBth, GM_ADDR A, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    const __gm__ Arch32ChunkGdnCoreFwdTrailer *phase6 = GetPhase6Trailer(tiling);
    Arch32ChunkGdnCoreFwdAbcTiling abc{};
...
{
    (void)a_storage;
    REGISTER_TILING_DEFAULT(GDN::Arch32ChunkGdnCoreFwdTrailer);
    if (TILING_KEY_IS(1)) {
        KERNEL_TASK_TYPE(1, KERNEL_TYPE_MIX_AIC_1_2);
        const __gm__ GDN::Arch32ChunkGdnCoreFwdTrailer *phase6 = GDN::GetPhase6Trailer(tiling);
