...
CORE = GDN_FWD / "chunk_gated_delta_rule_fwd"
ARCH22 = CORE / "op_kernel/internal/arch22"
ARCH35 = CORE / "op_kernel/internal/arch35"
ENTRY = CORE / "op_kernel/chunk_gated_delta_rule_fwd.cpp"
HOST_TILING = CORE / "op_host/chunk_gated_delta_rule_fwd_tiling.cpp"
HOST_CMAKE = CORE / "op_host/CMakeLists.txt"
OP_DEF = CORE / "op_host/chunk_gated_delta_rule_fwd_def.cpp"
OPTILING_CMAKE = ROOT / "cmake/obj_func.cmake"
ARCH22_ENTRY = ARCH22 / "chunk_gated_delta_rule_fwd_arch22.cpp"
ARCH35_ENTRY = ARCH35 / "chunk_gated_delta_rule_fwd_arch35.cpp"
ARCH22_STRUCT = ARCH22 / "chunk_gated_delta_rule_fwd_arch22_struct.h"
ARCH35_STRUCT = ARCH35 / "chunk_gated_delta_rule_fwd_arch35_struct.h"
ARCH22_TILING = (
    CORE / "op_host/op_tiling/arch22/chunk_gated_delta_rule_fwd_arch22_tiling.cpp"
)
ARCH35_TILING = (
    CORE / "op_host/op_tiling/arch35/chunk_gated_delta_rule_fwd_arch35_tiling.cpp"
)
ARCH35_STATE_TILING = (
    CORE
    / "op_host/op_tiling/arch35/"
    "chunk_gated_delta_rule_fwd_arch35_state_output_tiling.cpp"
)
ARCH22_STATE_TILING = (
    CORE
    / "op_host/op_tiling/arch22/"
    "chunk_gated_delta_rule_fwd_arch22_state_output_tiling.cpp"
)
ARCH22_FUSED_SOLVE = (
    ARCH22
...
def test_public_entry_and_host_are_thin_architecture_dispatchers():
    ...
    assert entry.count("internal/arch22/chunk_gated_delta_rule_fwd_arch22.cpp") == 1
    assert entry.count("internal/arch35/chunk_gated_delta_rule_fwd_arch35.cpp") == 1
    assert "internal/arch22/operators/" not in entry
    assert "internal/operators/" not in entry
    assert "__CCE_AICORE__ == 310" in entry

    assert "Tiling4ChunkGatedDeltaRuleFwdArch22(context)" in host
    assert "Tiling4ChunkGatedDeltaRuleFwdArch35(context)" in host
    assert "SocVersion::ASCEND950" in host
    assert "internal/" not in host

    assert "internal/arch22/operators" in cmake
...
def test_architecture_entries_and_host_tiling_are_isolated():
    arch22_entry = ARCH22_ENTRY.read_text(encoding="utf-8")
    arch35_entry = ARCH35_ENTRY.read_text(encoding="utf-8")
    arch22_tiling = ARCH22_TILING.read_text(encoding="utf-8")
    arch35_tiling = ARCH35_TILING.read_text(encoding="utf-8")

    assert arch22_entry.count('#include "operators/') == 2
    assert "arch35" not in arch22_entry
    assert "Arch22ChunkGatedDeltaRuleFwdTrailer" in arch22_entry
    assert "Tiling4ChunkGatedDeltaRuleFwdArch22StateOutput" in arch22_tiling
    assert "arch35" not in arch22_tiling

    assert '#include "../coefficient_generation/' in arch35_entry
    assert "arch22" not in arch35_entry
    assert "Arch35ChunkGatedDeltaRuleFwdTrailer" in arch35_entry
    assert "Tiling4ChunkGatedDeltaRuleFwdArch35StateOutput" in arch35_tiling
    assert "arch22" not in arch35_tiling


def test_arch22_contains_no_arch35_implementation_or_reference():
    files = tuple(
        path
        for path in ARCH22.rglob("*")
...
def test_arch22_fused_solve_uses_only_private_headers():
    source = ARCH22_FUSED_SOLVE.read_text(encoding="utf-8")
...
def test_frozen_a2_kernels_remain_private_and_a5_public_kernels_unchanged():
    for relative, expected in A2_KERNEL_SHA256.items():
        assert normalized_sha256(ARCH22 / "operators" / relative) == expected
    for relative, expected in A5_PUBLIC_SHA256.items():
        assert normalized_sha256(GDN_FWD / relative) == expected


def test_arch22_and_arch35_trailer_binary_layouts_match():
    arch22 = ARCH22_STRUCT.read_text(encoding="utf-8")
    arch35 = ARCH35_STRUCT.read_text(encoding="utf-8")

    assert struct_field_types(
        arch22, "Arch22ChunkGatedDeltaRuleFwdAbcTiling"
    ) == struct_field_types(
        arch35, "Arch35ChunkGatedDeltaRuleCoefficientTiling"
    )
    assert struct_field_types(
        arch22, "Arch22ChunkGatedDeltaRuleFwdTrailer"
    )[1:] == struct_field_types(
        arch35, "Arch35ChunkGatedDeltaRuleFwdTrailer"
    )[1:]


def test_arch22_state_tiling_keeps_frozen_a2_body():
    assert normalized_sha256(ARCH22_STATE_TILING) == (
        "1caaf7fe2a7a5a2c46c11b02632ba142ce2a45923b3c5500a9d1edac53d22124"
    )
...
def test_architecture_tiling_files_are_discovered_by_host_build():
    ...
    assert ARCH22_STATE_TILING.is_file()
    assert ARCH22_TILING.is_file()
    assert ARCH35_TILING.is_file()
    assert ARCH35_STATE_TILING.is_file()
    assert not (CORE / "op_host/chunk_gated_delta_rule_fwd_recompute_wu_tiling.cpp").exists()
    assert (
...
def test_public_name_and_private_architecture_names_are_clean():
    old_core_name = "chunk_" + "gdn_core_fwd"
    old_arch_path = "internal/" + "arch32"
    old_tiling_path = "op_tiling/" + "arch32"
    assert not (GDN_FWD / old_core_name).exists()
    tracked_text = "\n".join(
        path.read_text(encoding="utf-8")
        for path in CORE.rglob("*")
        if path.suffix in {".cpp", ".h", ".hpp", ".md", ".txt"}
        or path.name == "CMakeLists.txt"
    )
    assert old_core_name not in tracked_text
    assert old_arch_path not in tracked_text
    assert old_tiling_path not in tracked_text
    assert "chunk_gdn_fwd/solve_tri" not in tracked_text
