...
| 一次性通过 | 无 | 243 PASS（22 个算子被调用） | **910b：266 PASS / 0 FAIL（250 场景 + 11 条带原因 SKIP）；950：29 PASS / ALL PASS（12 场景 + 5 条带原因 SKIP）** |
...
regression_stable_a5.py   (950PR):   29 PASS / 0 FAIL   "ALL PASS: Ascend950 stable parity"
...
```json
{
  ...
  "Ascend950PR_9579":   {"passed": {"chunk_fwd_o(NTD)": 0.0, ...}, "skipped": {...}}
}
```
...
`ALL PASS`。当前记录：910B3 **250 通过 + 11 条带原因的 SKIP**，
950PR **29 通过 + 5 条带原因的 SKIP**（同一批算子跨 SOC 的公共部分见 §8.7）。
