...
| 每日 / 日常构建 | 不设（默认 `FALSE`） | `26.9.1+26.9.1.dev0a1b2c3`，即 `<版本>+<分支>_dev<commit7>` |

- 本地版本里的 `<分支>` 就是构建所在分支：`main` 分支是 `main_dev<commit>`（`26.7.0.dev0+main.dev0a1b2c3`），发布线去掉 `v` 前缀后用版本号本身（`26.9.1_dev<commit>`），其他开发分支用分支名（`my-branch_dev<commit>`）；
- PEP 440 会把下划线规范化成点号，所以文件里显示为 `+main.dev0a1b2c3` / `+26.9.1.dev0a1b2c3`；`pip` 也用规范化后的形式比较；
...
- 发布流程与门禁只认正式包：带本地版本的 wheel 上正式索引会被
  `scripts/check_pypi_wheel.py --require-release-version` 拦下来（可以照常发 TestPyPI）；手工构建可用 `FLA_NPU_LOCAL_VERSION` 覆盖本地版本、`FLA_NPU_COMMIT_ID` 覆盖 commit 标识（CI 里来自 `GITHUB_SHA`）。
