...
能验的那一部分还是验了：新增的静态检查逐个解析生成 wrapper 的 `_op("...")` 调用，
要求"实参个数 = schema 槽位数 + 1（stream）"。这条对**没有内核的算子同样有效**，
而且正是先前那次事故的对照检查——隐藏 ABI 槽被跳过时，后面的参数会整体错位成
"unable to cast 1 to Tensor"。26 个 wrapper 全部通过，包括跑不了的那两个。
