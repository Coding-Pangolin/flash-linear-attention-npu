...
小版本各发一份。触发与矩阵在 `.github/workflows/pypi.yml`，可移植性与水位登记在
[Stable-ABI 可移植性风险登记](architecture/stable-abi-portability-risks.md)。
...
- **发布线**（例如 `v26.9.1`）承载发布版本：发布前把 `__version__` 改成 `26.9.1`，与 v26.6.0
  当年的做法一致；
- **main** 保持 `<next>.dev0`；带 `.dev` 或本地版本（`+xxx`）的树只允许发 TestPyPI，
  `scripts/check_pypi_wheel.py --require-release-version` 会在正式索引前把它拦下来；
- 三个档位的 wheel 在不同机器上构建，因此版本必须来自树，不能按机器各写一份。

### 6.2 单机构建与门禁

```sh
export FLA_NPU_PYPI=TRUE FLA_NPU_DISABLE_LOCAL_VERSION=TRUE \
...
`.so` 断言 ELF 架构、glibc 水位（`manylinux_2_34`）以及"OPP host 库不得引用任何 `GLIBCXX_*`"
（后者靠 `cmake/intf_pub.cmake` 里的 `-static-libstdc++`，见风险登记 A1 节）。
