...
CHUNK_GATED_DELTA_RULE_FWD_CPP = (
    Path(__file__).resolve().parents[3]
    / "fla/ops/ascendc/gdn/chunk_gdn_fwd/chunk_gated_delta_rule_fwd/op_host/op_api/aclnn_chunk_gated_delta_rule_fwd.cpp"
)
CHUNK_GATED_DELTA_RULE_FWD_HEADER = CHUNK_GATED_DELTA_RULE_FWD_CPP.with_suffix(".h")
class FakeTensor:
...
class ChunkGatedDeltaRuleFwdCtypesAbiTest(unittest.TestCase):
    def test_example_uses_composite_core_by_default(self):
        source = EXAMPLE_PATH.read_text(encoding="utf-8")
        import ast

        self.assertIn("chunk_gated_delta_rule_fwd as ascendc_chunk_gated_delta_rule_fwd", source)
    ...
    def test_dense_call_matches_public_aclnn_signature(self):
        ...
        outputs = module.npu_chunk_gated_delta_rule_fwd(**make_inputs(), chunk_size=64, scale=0.125)
    ...
    def test_varlen_final_state_uses_initial_state_dtype(self):
        ...
        output, final_state, g_cumsum, A = module.npu_chunk_gated_delta_rule_fwd(
    ...
    def test_rejects_noncanonical_varlen_chunk_order(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "canonical sequence-major order"):
            module.npu_chunk_gated_delta_rule_fwd(
                **make_inputs(tokens=130),
                chunk_size=64,
                cu_seqlens=[0, 65, 130],
    ...
    def test_phase6_accepts_v256(self):
        ...
        output, final_state, g_cumsum, A = module.npu_chunk_gated_delta_rule_fwd(**inputs)
    ...
    def test_phase6_cpp_param_contract_allows_v256(self):
        source = CHUNK_GATED_DELTA_RULE_FWD_CPP.read_text(encoding="utf-8")
        self.assertIn("CheckParams(const ChunkGatedDeltaRuleFwdParams &params)", source)
        self.assertIn("vDim == CHUNK_GATED_DELTA_RULE_FWD_V256", source)
    ...
    def test_get_workspace_signature_matches_aclnn_header(self):
        ...
        self.assertEqual(module._GET_WORKSPACE_ARGTYPES["aclnnChunkGatedDeltaRuleFwd"], expected)

    def test_phase6_wrapper_uses_fixed_aclnn_symbol(self):
        captured = {}
        module = load_ctypes_module(captured)
        module.npu_chunk_gated_delta_rule_fwd(**make_inputs(), chunk_size=64)
        self.assertEqual(captured["name"], "aclnnChunkGatedDeltaRuleFwd")
        self.assertEqual(len(captured["args"]), 15)

    def test_cpp_route_keeps_only_phase6_boundary(self):
        source = CHUNK_GATED_DELTA_RULE_FWD_CPP.read_text(encoding="utf-8")
        header = CHUNK_GATED_DELTA_RULE_FWD_HEADER.read_text(encoding="utf-8")
        self.assertIn("l0op::ChunkGatedDeltaRuleFwd", source)
        self.assertIn("aclnnChunkGatedDeltaRuleFwdGetWorkspaceSize", source)
        self.assertIn("aclnnChunkGatedDeltaRuleFwdGetWorkspaceSize", header)
        self.assertIn("aclnnChunkGatedDeltaRuleFwd(", header)
