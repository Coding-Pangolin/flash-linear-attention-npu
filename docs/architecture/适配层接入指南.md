...
输出 shape 推导用 `size_of(<TensorMeta>, dim)`：坏 rank 时它把**读这个维度的那一行**和实际形状一起报出来
（`fla_npu(stable): size_of dim 1 out of range at csrc/src/stable_causal_conv1d_bwd.cpp:74 (ndim=1, shape=[1536])`），
没传的可选入参报 `(tensor is None / undefined)`。调用点不用传额外东西——file/line 是默认实参，由编译器在调用点
填；`layout_math.h` 里的 helper 会把调用点原样透传，新加 helper 照它的写法即可。
...
- 看到 `fla_npu(stable): size_of dim N out of range at csrc/src/xxx.cpp:LL (ndim=..., shape=[...])`：`LL` 就是
  按错的 rank 取维度的那一行（常见是把 4-D 输入当 3-D 传，或漏传了该给的张量），打开它就能对上是哪个入参；
  这里最后的 `ndim` / `shape` 描述的是**出错的那张张量自己**（它的维数与实际形状），不是第几个入参；
  按算子文档里该入参应有的 rank 改调用，别去放宽适配层的 shape 规则。
