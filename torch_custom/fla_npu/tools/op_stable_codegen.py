...
def generate_python(spec: dict) -> str:
    ...
    # The dispatcher call is emitted *positionally, in schema order*: the
    # earlier shape built a dict and had `_call` re-derive the order from _SIG on
    # every invocation, which costs a dict lookup per argument plus the dict
    # itself -- visible on the wide operators (19 tensor arguments) where it was
    # the whole gap to the pybind launcher (chunk_kda_fwd 1.30x).
    arguments = [
        f'{argument["name"]}' for argument in spec["args"]
        if argument["kind"] == "int_array" or argument["kind"] == "tensor"
    ]
    call_args: list[str] = []
    for argument in spec["args"]:
        if argument["kind"] == "out_tensor":
        ...
        if argument_name in hidden:
            # Hidden slots are required by the aclnn prototype but are not part
            # of the public signature; the reference passes None for them.  They
            # still occupy their position in the schema, so they are emitted as
            # None rather than skipped -- skipping shifts every argument after
            # them (which showed up as "unable to cast 1 to Tensor").
            call_args.append("None")
            continue
        if argument["kind"] == "int_array":
            call_args.append(f"_host_ints({argument_name})")
        elif argument["kind"] == "char_ptr":
            call_args.append(
                f'_char_code("{name}", "{argument_name}", {argument_name})')
        else:
            call_args.append(argument_name)
    call_args.append("_current_stream_ptr()")
    call = (f'_op("{name}")(\n' +
...
"""]
    ...
    py_lines = [
        '"""Generated by tools/op_stable_codegen.py -- do not edit by hand."""',
        # The wrappers call the op positionally, so they need the cache handle,
        # the int-array/char-ptr encoders and the stream accessor directly.
