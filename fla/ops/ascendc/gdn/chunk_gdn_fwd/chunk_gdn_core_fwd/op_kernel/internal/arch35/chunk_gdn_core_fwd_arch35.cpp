 ...
 */
#include "chunk_gdn_core_fwd_arch35_struct.h"

#include "../state_update_output/chunk_gdn_core_state_update_output.cpp"
#include "../coefficient_generation/chunk_gdn_core_coefficient_generation.cpp"
...
__aicore__ inline const __gm__ Arch35ChunkGdnCoreFwdTrailer *GetPhase6Trailer(GM_ADDR tiling)
{
    const uint64_t oTilingOffset = AlignPhase6(
        sizeof(ChunkGatedDeltaRuleFwdHTilingData), PHASE6_TILING_ALIGNMENT);
    const uint64_t stateOutputTilingEnd = oTilingOffset + sizeof(ChunkFwdOTilingData) +
                                          sizeof(ChunkGdnCoreStateOutputTrailer);
    return reinterpret_cast<const __gm__ Arch35ChunkGdnCoreFwdTrailer *>(
        tiling + AlignPhase6(stateOutputTilingEnd, PHASE6_TILING_ALIGNMENT));
}

__aicore__ inline void CopyCoefficientTiling(
    const __gm__ Arch35ChunkGdnCoreCoefficientTiling *src, Arch35ChunkGdnCoreCoefficientTiling &dst)
{
    dst.B = src->B;
    dst.Hk = src->Hk;
    dst.Hv = src->Hv;
...
__aicore__ inline void WritePublicCumsumRows(
    ...
    const Arch35ChunkGdnCoreCoefficientTiling &tiling)
{
    if ASCEND_IS_AIC {
        return;
    }
    ...
    GM_ADDR finalState, GM_ADDR gCumsumBth, GM_ADDR A, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    const __gm__ Arch35ChunkGdnCoreFwdTrailer *phase6 = GetPhase6Trailer(tiling);
    Arch35ChunkGdnCoreCoefficientTiling coefficient{};
...
{
    (void)a_storage;
    REGISTER_TILING_DEFAULT(GDN::Arch35ChunkGdnCoreFwdTrailer);
    if (TILING_KEY_IS(1)) {
        KERNEL_TASK_TYPE(1, KERNEL_TYPE_MIX_AIC_1_2);
        const __gm__ GDN::Arch35ChunkGdnCoreFwdTrailer *phase6 = GDN::GetPhase6Trailer(tiling);
