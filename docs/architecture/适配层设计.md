...
7. **取单个维度用 `SIZE_OF(meta, dim)`**（整条 shape 原样照搬时直接传 `meta.sizes` 没问题）：函数体里看不到实参文本，所以适配层用 `SIZE_OF` 宏把 `#meta` 和调用点的 `__builtin_FILE/LINE` 一起带进异常——报错同时给出**是哪张张量**、**读这个维度的那一行**和实际 `ndim`/`shape`，输出 shape 规则每个维度都要取一次，报错必须能指到算子里是哪句。`layout_math.h` 的 helper 仍调 `size_of(meta, dim, file, line)` 函数并透传调用点，锚点同样是算子自己那行，只是不带张量名；新写 helper 时照抄这个模式。
