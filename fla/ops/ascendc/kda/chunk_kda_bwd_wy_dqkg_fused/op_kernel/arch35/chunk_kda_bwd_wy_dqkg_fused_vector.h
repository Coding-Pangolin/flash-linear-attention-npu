 ...
 *
 * ChunkKdaBwdWyDqkgFused — Ascend950 / arch35 Vector (AIV) side.
 *
 * Schedule mirrors 910B (DESIGN §5.1). Arithmetic hot paths use MicroAPI
 * RegTensor via chunk_kda_bwd_wy_dqkg_fused_regbase.h (regbase).
 *
...
#include "chunk_kda_bwd_wy_dqkg_fused_regbase.h"
...
public:
                                ...
                                TPipe *pipe)
    {
...
#if USE_MASK_SELECT_SLIM
        {
            ...
            regbase_ops::DuplicateReg(zero, 0.0f, 8);
            PipeBarrier<PIPE_V>();
        }
#endif
#if USE_VEC_MTE2_PP
...
private:
    ...
    // Ascend950 MicroAPI (regbase) arithmetic helpers.
    __aicore__ inline void Exp2InPlace(LocalTensor<float> t, uint32_t count)
    {
        regbase_ops::Exp2InPlaceReg(t, count, LN2, EXP_INPUT_MAX, EXP_INPUT_MIN);
    }

    // dst[r,c] *= rowVec[r]. brcbScratch unused on A5 (kept for call-site parity).
    ...
    {
        (void)brcbScratch;
        regbase_ops::RowBroadcastMulInPlaceReg(dst, rowVec, rows, cols);
    }
    ...
    __aicore__ inline void ColBroadcastMulInPlace(LocalTensor<float> &dst, LocalTensor<float> &colVec, uint32_t rows,
                                                  uint32_t cols)
    {
        regbase_ops::ColBroadcastMulInPlaceReg(dst, colVec, rows, cols);
    }

    // Reduce mat[rows,width] along last axis; add into acc[rows] (MicroAPI ReduceSum).
    __aicore__ inline void RowFoldSumAddInto(LocalTensor<float> &acc, LocalTensor<float> &mat, uint32_t rows,
                                             uint32_t width)
    {
        regbase_ops::RowFoldSumAddIntoReg(acc, mat, rows, width);
    }
                                         ...
                                         uint32_t width)
    {
        ...
        while (n > 1) {
            ...
            regbase_ops::AddInPlaceReg(mat, mat, mat[half * width], half * width);
            PipeBarrier<PIPE_V>();
            if ((n & 1U) != 0U) {
                regbase_ops::AddInPlaceReg(mat, mat, mat[(n - 1) * width], width);
                PipeBarrier<PIPE_V>();
            }
            n = half;
        }
        regbase_ops::AddInPlaceReg(acc, acc, mat, width);
                                                  ...
                                                  uint32_t width)
    {
        ...
        regbase_ops::DuplicateReg(acc, 0.0f, static_cast<uint32_t>(bt_));
        PipeBarrier<PIPE_V>();
        LocalTensor<float> matOwned = mat[r0 * width];
    ...
    __aicore__ inline void MergeDbAcc(uint64_t slot)
    {
        ...
        regbase_ops::DuplicateReg(dbAcc, 0.0f, static_cast<uint32_t>(bt_));
        PipeBarrier<PIPE_V>();
        for (uint32_t s = 0; s < subBlockNum_ && s < 2; ++s) {
            ...
            SyncMte2ToV();
            regbase_ops::AddInPlaceReg(dbAcc, dbAcc, tmp, static_cast<uint32_t>(bt_));
            PipeBarrier<PIPE_V>();
            if (s + 1 < subBlockNum_) {
                SyncVToMte2();
            }
    ...
    __aicore__ inline void MergeDgkAcc(uint64_t slot, LocalTensor<float> &dgkAcc, uint32_t bk)
    {
        ...
        regbase_ops::DuplicateReg(dgkAcc, 0.0f, bk);
        PipeBarrier<PIPE_V>();
        for (uint32_t s = 0; s < subBlockNum_ && s < 2; ++s) {
            DataCopy(tmp, wsF32_[f32Base + SlotLayoutF32::dgkMergeWs + static_cast<uint64_t>(s) * MAX_BK], bk);
            SyncMte2ToV();
            regbase_ops::AddInPlaceReg(dgkAcc, dgkAcc, tmp, bk);
            PipeBarrier<PIPE_V>();
            if (s + 1 < subBlockNum_) {
                SyncVToMte2();
            }
                                             ...
                                             uint64_t validRows, uint64_t vOff, uint32_t bv)
    {
        ...
        regbase_ops::CastTToF32Reg(aFp, aIn, aElems);
        regbase_ops::CastTToF32Reg(dvFp, dvInT, dvElems);
        PipeBarrier<PIPE_V>();
        regbase_ops::DuplicateReg(dvb, 0.0f, static_cast<uint32_t>(bt_) * bv);
        PipeBarrier<PIPE_V>();
        LocalTensor<float> rowTmp = dvFp[dvElems];
        // dvb[r,:] += A[r,k] * dv[k,:]  (MicroAPI BRC × row)
        for (uint32_t kk = 0; kk < vr; ++kk) {
            LocalTensor<float> dvRow = dvFp[kk * bv];
            for (uint32_t r = 0; r < vr; ++r) {
                __ubuf__ float *aScal = (__ubuf__ float *)aFp.GetPhyAddr() + r * vr + kk;
                __ubuf__ float *dvAddr = (__ubuf__ float *)dvRow.GetPhyAddr();
                __ubuf__ float *tmpAddr = (__ubuf__ float *)rowTmp.GetPhyAddr();
                __ubuf__ float *dvbAddr = (__ubuf__ float *)dvb.GetPhyAddr() + r * bv;
                uint32_t remain = bv;
                const uint16_t loops = regbase_ops::CeilDivU16(bv, regbase_ops::VL_F32);
                __VEC_SCOPE__
                {
                    using namespace AscendC::MicroAPI;
                    RegTensor<float> scal, src, acc, out;
                    MaskReg preg;
                    LoadAlign<float, LoadDist::DIST_BRC_B32>(scal, aScal);
                    for (uint16_t c = 0; c < loops; ++c) {
                        preg = UpdateMask<float>(remain);
                        DataCopy(src, dvAddr + c * regbase_ops::VL_F32);
                        Mul(out, src, scal, preg);
                        DataCopy(tmpAddr + c * regbase_ops::VL_F32, out, preg);
                        DataCopy(acc, dvbAddr + c * regbase_ops::VL_F32);
                        Add(acc, acc, out, preg);
                        DataCopy(dvbAddr + c * regbase_ops::VL_F32, acc, preg);
                    }
                }
            }
        }
                                     ...
                                     uint32_t nBv)
    {
        ...
        SetFlag<HardEvent::MTE2_V>(EVT_MTE2_V);
        WaitFlag<HardEvent::MTE2_V>(EVT_MTE2_V);
        regbase_ops::CastTToF32Reg(beta, betaIn, static_cast<uint32_t>(bt_));
        PipeBarrier<PIPE_V>();
        ...
        if (IsSub0()) {
            ...
            for (uint32_t iv = 1; iv < nBv; ++iv) {
                ...
                DataCopy(tmp, wsF32_[f32Base + SlotLayoutF32::dASlot + static_cast<uint64_t>(iv) * MAX_BT * MAX_BT],
                         btbt);
                SyncMte2ToV();
                regbase_ops::AddInPlaceReg(dAWs, dAWs, tmp, btbt);
                PipeBarrier<PIPE_V>();
            }
            SyncVToMte3();
        ...
        for (uint32_t iv = 0; iv < nBv; ++iv) {
            ...
            if (partialBt) {
                ...
                regbase_ops::CastTToF32Reg(vFp, vIn, elems);
                PipeBarrier<PIPE_V>();
            ...
            regbase_ops::CastTToF32Reg(vFp, vIn, ownedElems);
            PipeBarrier<PIPE_V>();
            ...
            regbase_ops::MulInPlaceReg(prod, dvb, vFp, ownedElems);
            PipeBarrier<PIPE_V>();
            LocalTensor<float> dbOwned = dbAcc[r0];
            RowFoldSumAddInto(dbOwned, prod, nr, bv);

            LocalTensor<T> dv2T = arenaT_.Get<T>();
            regbase_ops::CastF32ToTReg(dv2T, dv2Fp, ownedElems);
            PipeBarrier<PIPE_V>();
            SetFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
            WaitFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
                                 ...
                                 uint64_t validRows, uint32_t iK)
    {
        ...
        if (r0 >= static_cast<uint32_t>(bt_) || nr == 0) {
            if (IsSub0()) {
                ...
                regbase_ops::CastTToF32Reg(gnFp, gnIn, bk);
                PipeBarrier<PIPE_V>();
                SyncVToMte3();
        ...
        regbase_ops::CastTToF32Reg(kOwned, kIn, nElem);
        regbase_ops::CastTToF32Reg(gOwned, gIn, nElem);
        LocalTensor<float> gnFp = smallBuf_.Get<float>();
        regbase_ops::CastTToF32Reg(gnFp, gnIn, bk);
        PipeBarrier<PIPE_V>();
        ...
        regbase_ops::MulInPlaceReg(kgOwned, kOwned, gkOwned, nElem);
        PipeBarrier<PIPE_V>();
        LocalTensor<T> kgT = arenaT_.Get<T>()[3 * btbk];
        LocalTensor<T> kgTOwned = kgT[rowBase];
        regbase_ops::CastF32ToTReg(kgTOwned, kgOwned, nElem);
        PipeBarrier<PIPE_V>();
        SetFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
        WaitFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
        if (IsSub0()) {
                                       ...
                                       uint64_t localChunk, uint64_t validRows, uint32_t nBv, uint32_t iK)
    {
        ...
        if (nr > 0) {
            ...
            regbase_ops::MulInPlaceReg(dqOwned, dqOwned, gkOwned, nElem);
            regbase_ops::MulsInPlaceReg(dqOwned, dqOwned, scale_, nElem);
            PpAcquire(sDk);
            PpAcquire(sDw);
#else
...
#endif
#else
            regbase_ops::DuplicateReg(dqOwned, 0.0f, nElem);
            regbase_ops::DuplicateReg(dkOwned, 0.0f, nElem);
            regbase_ops::DuplicateReg(dwOwned, 0.0f, nElem);
            PipeBarrier<PIPE_V>();
            for (uint32_t iv = 0; iv < nBv; ++iv) {
                ...
                SyncMte2ToV();
                regbase_ops::AddInPlaceReg(dqOwned, dqOwned, tmp, nElem);
                PipeBarrier<PIPE_V>();
                SyncVToMte2();
                DataCopy(tmp, wsF32_[f32Base + SlotLayoutF32::dkSlot + off], nElem);
                SyncMte2ToV();
                regbase_ops::AddInPlaceReg(dkOwned, dkOwned, tmp, nElem);
                PipeBarrier<PIPE_V>();
                SyncVToMte2();
                DataCopy(tmp, wsF32_[f32Base + SlotLayoutF32::dwSlot + off], nElem);
                SyncMte2ToV();
                regbase_ops::AddInPlaceReg(dwOwned, dwOwned, tmp, nElem);
                PipeBarrier<PIPE_V>();
                if (iv + 1 < nBv) {
                    SyncVToMte2();
                }
        ...
        if (nr > 0) {
            ...
            regbase_ops::MulInPlaceReg(dkOwned, dkOwned, expOwned, nElem);
            PipeBarrier<PIPE_V>();
        }
        SyncVToMte3();
        ...
        if (nr > 0) {
            ...
            regbase_ops::MulInPlaceReg(kdkOwned, kOwned, dkOwned, nElem);
            PipeBarrier<PIPE_V>();
        }
        ...
        if (nr > 0) {
            regbase_ops::MulsInPlaceReg(dwOwned, dwOwned, -1.0f, nElem);
            PipeBarrier<PIPE_V>();
            LocalTensor<T> dwNegT = arenaT_.Get<T>()[4 * btbk];
            LocalTensor<T> dwNegOwned = dwNegT[rowBase];
            regbase_ops::CastF32ToTReg(dwNegOwned, dwOwned, nElem);
            PipeBarrier<PIPE_V>();
            SetFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
            WaitFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
                                     ...
                                     uint64_t localChunk, uint64_t validRows, uint32_t nBv, uint32_t iK)
    {
        ...
        if (nr > 0) {
            ...
            PpAcquire(sDkgb);
            PpAcquire(sKg);
            regbase_ops::CastTToF32Reg(kgFp[rowBase], kgIn, nElem);
            PipeBarrier<PIPE_V>();
            ...
            if (validOwned < nr) {
                ...
                regbase_ops::CastTToF32Reg(qFp[rowBase], qIn, nElem);
                regbase_ops::CastTToF32Reg(kFp[rowBase], kIn, nElem);
                PpAcquire(sDk);
                PpAcquire(sGk);
                PpAcquire(sDq);
                PipeBarrier<PIPE_V>();
        ...
        if (nr > 0) {
            ...
            LocalTensor<float> kgOwned = kgFp[rowBase];
            regbase_ops::MulInPlaceReg(scratchOwned, dkgbOwned, kgOwned, nElem);
            PipeBarrier<PIPE_V>();
#if USE_OWNED_ARENA
        ...
        if (nr > 0) {
            ...
            regbase_ops::MulInPlaceReg(kOwned, kOwned, dkOwned, nElem);
#if USE_FOLD_BAR_SLIM
            // Independent dsts — one barrier before Sub that reads both.
            regbase_ops::MulInPlaceReg(qOwned, qOwned, dqOwned, nElem);
            PipeBarrier<PIPE_V>();
#else
            PipeBarrier<PIPE_V>();
            regbase_ops::MulInPlaceReg(qOwned, qOwned, dqOwned, nElem);
...
#endif
            if (OwnRow(lastRow)) {
                regbase_ops::AddInPlaceReg(qOwned[(lastRow - r0) * bk], qOwned[(lastRow - r0) * bk], dgkRow, bk);
                PipeBarrier<PIPE_V>();
            }

            regbase_ops::MulInPlaceReg(kOwned, kgOwned, dkgbOwned, nElem);
            PipeBarrier<PIPE_V>();
            LocalTensor<float> beta = betaBuf_.Get<float>();
#if USE_VEC_MTE2_PP && USE_VEC_MTE2_PP_EPILOG
            {
            ...
            regbase_ops::AddInPlaceReg(qOwned, qOwned, kOwned, nElem);
            PipeBarrier<PIPE_V>();

            RowBroadcastMulInPlace(gkOwned, betaOwned, brcbScratch, nr, bk);
            regbase_ops::MulInPlaceReg(dkgbOwned, dkgbOwned, gkOwned, nElem);
            PipeBarrier<PIPE_V>();
            regbase_ops::AddInPlaceReg(dkOwned, dkOwned, dkgbOwned, nElem);
#if USE_EPILOG_STORE_MERGE
            SetFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
            WaitFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
#else
        ...
        if (lastBk && IsSub0()) {
...
#if USE_VEC_MTE2_PP && USE_VEC_MTE2_PP_EPILOG
            {
                ...
                PpAcquire(sDa);
                PpAcquire(sDel);
                regbase_ops::AddInPlaceReg(dAWs, dAWs, delta, btbt);
            }
#else
        ...
        if (OwnRow(lastRow)) {
            ...
            regbase_ops::DuplicateReg(stateAcc, 0.0f, bk);
            PipeBarrier<PIPE_V>();
            ...
            for (uint32_t iv = 0; iv < nBv; ++iv) {
                ...
                if (stateVFirst_) {
                    ...
                    regbase_ops::CastTToF32Reg(hFp, hIn, bvbk);
                    regbase_ops::CastTToF32Reg(dhFp, dhIn2, bvbk);
                    PipeBarrier<PIPE_V>();
                    regbase_ops::MulInPlaceReg(hFp, hFp, dhFp, bvbk);
                    PipeBarrier<PIPE_V>();
                    ...
                    regbase_ops::CastTToF32Reg(hFp, hPanel, bkbv);
                    regbase_ops::CastTToF32Reg(dhFp, dhPanel, bkbv);
                    PipeBarrier<PIPE_V>();
                    regbase_ops::MulInPlaceReg(hFp, hFp, dhFp, bkbv);
                    PipeBarrier<PIPE_V>();
                    ...
                    for (uint32_t r = 0; r < bk; ++r) {
                        ...
                        SetFlag<HardEvent::MTE2_V>(EVT_STATE_MTE2_V);
                        WaitFlag<HardEvent::MTE2_V>(EVT_STATE_MTE2_V);
                        regbase_ops::CastTToF32Reg(hRow, hRowT, bv);
                        regbase_ops::CastTToF32Reg(dhRow, dhRowT, bv);
                        PipeBarrier<PIPE_V>();
                        regbase_ops::MulInPlaceReg(hRow, hRow, dhRow, bv);
                        PipeBarrier<PIPE_V>();
                        uint32_t remain = bv;
                        while (remain > 8) {
                            const uint32_t calcCnt = remain / 2;
                            const uint32_t newRemain = remain - calcCnt;
                            regbase_ops::AddInPlaceReg(hRow, hRow, hRow[newRemain], calcCnt);
                            PipeBarrier<PIPE_V>();
                            remain = newRemain;
                        }
            ...
            regbase_ops::MulInPlaceReg(stateAcc, stateAcc, gnExp, bk);
            PipeBarrier<PIPE_V>();
#if USE_OWNED_ARENA
            ...
            DataCopy(qFp[rowBase],
                     wsF32_[f32Base + SlotLayoutF32::dkPartialWs + static_cast<uint64_t>(r0) * bk], nElem);
            SyncMte2ToV();
            regbase_ops::AddInPlaceReg(qFp[rowBase + (lastRow - r0) * bk], qFp[rowBase + (lastRow - r0) * bk], stateAcc, bk);
#else
            regbase_ops::AddInPlaceReg(qFp[lastRow * bk], qFp[lastRow * bk], stateAcc, bk);
#endif
#if !USE_EPILOG_STORE_MERGE
            PipeBarrier<PIPE_V>();
#endif
    ...
    __aicore__ inline void Stage3MaskVec(uint64_t slot, uint64_t bIdx, uint64_t iHv, uint64_t validRows)
    {
        ...
        regbase_ops::CastF32ToTReg(dATOwned, dAOwned, nElem);
#if !USE_MASK_SELECT_SLIM
        PipeBarrier<PIPE_V>();
#endif
        SetFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
        ...
        LocalTensor<T> dAT = arenaT_.Get<T>();
        regbase_ops::CastF32ToTReg(dAT, dA, btbt);
#if !USE_MASK_SELECT_SLIM
        PipeBarrier<PIPE_V>();
#endif
        SetFlag<HardEvent::V_MTE3>(EVT_V_MTE3);
                                          ...
                                          uint64_t validRows)
    {
        ...
        if (nr > 0) {
            ...
            regbase_ops::MulsInPlaceReg(dA3Owned, dA3Owned, -1.0f, nElem);
            PipeBarrier<PIPE_V>();
#if !USE_MASK_ONCE
            LocalTensor<uint8_t> mask = EnsureMask(static_cast<uint32_t>(validRows));
#if USE_MASK_SELECT_SLIM
        ...
        SyncMte2ToV();
        regbase_ops::MulsInPlaceReg(dA3, dA3, -1.0f, btbt);
        PipeBarrier<PIPE_V>();
#if !USE_MASK_ONCE
        LocalTensor<uint8_t> mask = EnsureMask(static_cast<uint32_t>(validRows));
