...
|------|--------------|----------|
| 仅 P0-A 驻留 | −0.8 | ~3.8 |
| P0-A + P0-B S2c | −0.8−1.0 | ~2.8~3.2 |
| 再 + S4/prefetch/Store 重叠 | −0.3~−0.7 | ~2.3~2.8 |
| 仍 >1.5 | 需融合 / 改调度 / 减工作量 | **另立项** |

**诚实上限：** 在「4096 task × NC4 × 现有 Prep+Score+MCH+Store」语义不变时，**2.0~2.5 ms** 比 **1.5 ms** 更像本算子内可达带；1.5 作为 **牵引目标**，用工具打脸或证实需要架构刀。

---

## 7. 执行顺序（与 `SCORE_TILE_CROSSCORE_PLAN.md` 对齐）

```text
T0–T2  已完成：-g / Score Tile / CrossCore（板端最佳 ~4.402 ms）
T3     已完成：带 -g 的 T=1024 sim（code_exe 有行号；instr 仍 BAR/Nd2Nz/FIX）
T4     P0-A MCH 真 L1 驻留（精度 → msprof）  ← 下一刀
T5     P0-B S2c 批 MMAD/MCH（驻留后重做 Prep overlap）
T6     P1 S4/Store 重叠；驻留后若 mte2 仍高再 soft-prefetch
T7     若仍 ≫1.5：写「单算子天花板」+ 融合/分核另立项
```
