...
| `torch_custom/fla_npu/csrc/src/stable_<family>.cpp` | 该算子族的 Stable-ABI 适配（`kSchema_<op>` + `run_<op>`） |
| `torch_custom/fla_npu/fla_npu/ops/ascendc/_stable.py` | 薄层的 Python 真签名 wrapper（默认入口） |
...
├── csrc/src/stable_<family>.cpp    # 新增 npu_chunk_bwd_dv_local 的 kSchema + run_（一条宏下发）
├── csrc/src/stable_ops.cpp         # 注册两行：m.def + m.impl
└── fla_npu/ops/ascendc/
    ├── _stable.py                  # 新增 npu_chunk_bwd_dv_local(...) 薄层 wrapper
...
默认后端是 `torch_custom/fla_npu/csrc` 编译出的 Stable-ABI 薄层（`libfla_npu_stable.so`），
核心链路：

1. 在 `torch_custom/fla_npu/csrc/src/stable_<family>.cpp` 中写 `kSchema_<op>` + `run_<op>`：
   申请输出，再用一条 `FLA_STABLE_EXEC(...)` 按 `aclnn_xxx.h` 的实参顺序下发。
2. 在 `torch_custom/fla_npu/csrc/src/stable_ops.cpp` 注册两行：
   `m.def(kSchema_<op>);` 与 `m.impl("<op>", &boxed_adapter<run_<op>>);`。
3. 在 `torch_custom/fla_npu/fla_npu/ops/ascendc/_stable.py` 加一个真签名 wrapper
   （`_op("npu_xxx")(...)`）；公开名自动导出 `npu_xxx` 及去掉 `npu_` 前缀的短名。
   算子若会原地修改输入，再在 `__init__.py` 的 `MUTATED_ARGUMENTS` 登记参数名。
4. 在 `tests/stable_abi/regression_ops.py` 加一个 ctypes ↔ 薄层的逐位 parity 场景。
5. 重新构建 wheel / run 包并安装验证。

`_aclnn_ctypes.py` 保留为**参考实现与回退**：有同名 ctypes 函数时，门禁会拿它比对公开
签名和 host 封装；新算子不必以它为主路径。

详细步骤（含示例骨架、参数类型对照、族文件归属、门禁命令）见
[`torch_custom/fla_npu/README.md`](../torch_custom/fla_npu/README.md)；
设计取舍见 [`architecture/stable-abi-macro-design.md`](architecture/stable-abi-macro-design.md)。
