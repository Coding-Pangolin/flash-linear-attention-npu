...
| `csrc/include/stable/acl_meta.h` | `TensorMeta`、`AclTensorView`、`AclIntArrayView`、分配/元数据读取、带调用点位置取维度的 `size_of` |
...
7. **取单个维度用 `size_of(meta, dim)`**（整条 shape 原样照搬时直接传 `meta.sizes` 没问题）：函数体里看不到实参名，所以 `size_of` 用默认实参（调用点求值的 `__builtin_FILE/LINE`）把**读这个维度的那一行**和实际 `ndim`/`shape` 一起带进异常——输出 shape 规则每个维度都要取一次，报错必须能指到算子里是哪句。`layout_math.h` 的 helper 以同样的默认实参透传，锚点仍是算子自己那行；新写 helper 时照抄这个模式。
